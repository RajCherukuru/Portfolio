    ...
    const {country, startDate, endDate, company, role, desc, skills}= work;
     ...
     return (
        ...
        whileTap={{ scale: 0.9 }}  initial={{scale:0.7}} whileInView={{scale:1}} className="relative w-8/12  mx-auto font-mono">
                ...
                <div className=" relative z-10 w-full mx-auto flex flex-col gap-4 bg-black rounded-3xl p-5 border ">
                ...
                <ul className="list-disc text-white list-inside text-sm font-sans ">
                    {
                        desc.map((d, index) =>(
                            <li key={index}>{d}</li>
                        ))
